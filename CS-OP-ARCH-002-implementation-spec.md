...
- **ADR-30 — The invoicing grid is client-rendered, like every other screen.** Departs from §7, which specifies a server-rendered `<table>` from `render.py` for this screen. `render.py` does not exist, and building it for one grid means maintaining two rendering paradigms, two escaping stories and two sets of guardrails for the rest of the project's life. **The invariant §7 was protecting is preserved exactly:** no optimistic UI — every action sends, waits, and repaints from what the server returned, so the grid shows what happened rather than what was asked for. That is the property that stops a month being closed on figures nobody checked, and it does not depend on where the HTML is assembled. Accepted cost: §7's server-rendered path stays unbuilt, so if a later screen genuinely needs it — a printable report, or a grid too large to ship as JSON — `render.py` and its escape-by-default helpers have to be written then, and the §7 guardrail is already specified for that day. Revisit at STP-5, where drill-through reports may make the case properly.
- **ADR-31 — Workbook data is read from CSV exports only.** The Google Drive markdown conversion is not a data source and must not be used to derive a figure. On 25 Aug 2026 it dropped rows from Future Invoicing, merged an unrelated monthly snapshot into the Invoicing tab, and reported 131 rows where the CSV export has 147 — producing a $126,268.91 discrepancy that did not exist, two importer rules written to handle artifacts (a "copy-forward residue" that was really a merged tab, and a $1.15m "overlap" between tabs that do not overlap at all), and an hour spent telling the Ops Manager his workbook was incomplete. **The diagnostic that should have caught it immediately:** the pivot inside the same export already accounted for rows the detail tab appeared to lack, which is only possible if the reader is wrong rather than the data. When a control total disagrees with its own detail, suspect the reader first — and note that two independent parses agreeing proves nothing when both read the same corrupted copy. The conversion remains useful for reading structure and finding which tabs exist; it is never the source of a number.
- **ADR-32 — Moving a FORECAST claim between months needs no reason; moving a committed one does.** The first rule required a reason for every change of `period_id`, on the grounds that it was slippage. That is right for a claim which is `due` or beyond — it was committed to a month and has moved — and wrong for a `forecast`, where moving work between months *is* the forecasting. Requiring a justification for each one would have made re-forecasting unusable, which is the opposite of the point. **Either way the move is recorded**: a `claim_line_revision` row still shows the month it came from, so re-forecasting stays reviewable; it simply is not interrogated. The boundary is the moment a claim leaves `forecast`, because that is when someone outside the platform has been told to expect an invoice.
- **ADR-33 — The JS budget is what ONE PAGE weighs, not what the folder weighs.** §14's 50 KB limit was enforced as the sum of every file, with a test comment admitting why: every screen was statically imported by the shell, so the register paid for the invoicing grid it never showed. Screens now load with dynamic `import()`, and the gate walks the static-import graph to weigh each page separately (shell + screen + that screen's dependencies). Heaviest page at the time of writing: 35 KB. **The mechanism is itself guarded** — the shell must not statically import a screen, and no screen may import another — because the only symptom of that regressing is a slow first paint nobody attributes to it. Raising the number would have been the easy move and would have concealed the real problem, which was that the budget was measuring the wrong thing.
- **ADR-34 — The contract value belongs to the project; customer POs are what has actually been ordered.** Corrects a modelling error introduced by migration 003. The register's `Purchase Order` column was never a customer order: it was the CONTRACT VALUE, what the job is worth. Turning it into a `customer_po` row meant that on a project where orders arrive progressively, adding the real POs alongside the migrated one double-counted — `200 Victoria - IBP` read $422,833.33 against a $295,000 contract, because four orders that were portions OF the contract were summed WITH it. **Orders in hand keeps its original meaning, `contract − invoiced`**, which every pinned figure reconciles to; the PO-sum version had quietly redefined it. `ordered_cents` is the new figure and one the workbook could not give: how much of a contract the customer has actually raised an order for, and therefore what is not yet ordered. The migrated rows could not simply be deleted — 204 claims reference them and the retention terms sit on them — so they carry `is_placeholder = 1`, hold their claims, and are excluded from `ordered`. **Retention belongs to the PO** (a variation raising an order raises its cap; scope run separately carries its own terms or none), so a project may have retention on one order and not another. Consequence: the contraction migration that would have dropped `project.purchase_order_cents` is off — the contract value is a live field again, not legacy. **How it was caught:** the Ops Manager noticed a contract figure that had grown without anything being agreed. Nothing in the tests could have found it, because every test asserted the model as designed.
- **ADR-35 — Static assets are `no-store`, and the disk has its own fingerprint.** Two separate corrections to the same afternoon's confusion. First, static files were served `Cache-Control: no-cache` with no ETag and no Last-Modified: `no-cache` permits STORING and merely requires revalidation, and with no validator there is nothing to revalidate against — an ambiguous state browsers resolve differently, which left a module correct on disk running as an old copy in the tab. They are `no-store` now, which is also the consistent choice for an asset set the code already calls tiny and internal. Second, `code_fingerprint` deliberately excludes `static/` because those files are read per request, so the running server is never stale with respect to them — correct, and an answer to the wrong question. `asset_fingerprint` answers the other one: is the disk what was delivered. `/healthz` reports both and `dev.ps1 -Stale` prints both, because "am I current" had become a deduction rather than a comparison.
- **ADR-36 — One dialog helper, and no `window.prompt` for input.** `sheet.js` holds the modal and its field; three screens had grown their own and drifted. The invoicing grid raised a customer order through two prompts, which silently recorded LESS than the identical action on the register — no issue date, no note, no field-level errors — and the job-code correction collected its reason, which someone reads later, in a single-line box with no context. Where an action is taken from should change where it POSTs, not what can be recorded while doing it. A guardrail fails the build on any `window.prompt`; `window.confirm` stays, because a destructive yes/no is exactly what it is for.
- **ADR-37 — A contract becomes a forecast through ITEMS spread across MONTHS.** Migrations `008`–`010`. Until this the claim line was the atom and there was no way to make one: every claim arrived from the importer, so a project created here could not have a dollar of its invoicing planned. Rebuilt from the real progress-claim workbooks: a contract splits into ITEMS with values; each item is spread across months by PERCENTAGE; a month's claim is the sum of that month's contributions. `720 Bourke` reproduces to the cent. Three properties the workbooks check by hand are now continuous — items sum to what is left to claim, each item's allocations reach 100%, the cumulative lands on 100% — but **none is enforced**: a plan under construction is legitimately incomplete, and refusing to save it would mean building the whole thing in one sitting. The AMOUNT is the fact and the percentage is provenance: `33.33%` of $79,444 is $26,478.69 while the agreed figure is $26,481.33, so deriving one from the other at read time would move money. **Invoiced months are fixed** — the same boundary as slippage — with a deliberate amend path that records what the invoice SAID as well as what it says now, because reconciling to Xero means matching the figure that was issued.
- **ADR-38 — A claim line is one contribution; an INVOICE groups them.** Corrects `one claim per project per month`, which I invented and the register disproves twice over: `200 Victoria - IBP` has five Sep-26 claims, and two Aug-26 claims sharing `Inv No. 6072/5`. Three bugs came from that one assumption, all found by the Ops Manager using the screens rather than by any test. Generation updated one of five claims to the month's whole total and left four standing — **$88,500 of forecast became $159,300, in silence**. Moving one claim relocated the entire month's allocation. And moving it back was refused as `occupied`, so the plan silently stayed while the claim returned. Each allocation now owns its claim (`claim_allocation.claim_line_id`), and `UNIQUE (claim_item_id, period_id)` is gone — it said an item contributes to a month at most once, which is false.
- **ADR-39 — A plan is measured against what is LEFT to claim.** `720 Bourke`: a $198,610 contract with $112,545.67 invoiced before FY27. Comparing items to the whole contract called that money unitemised, but it is BILLED and no plan in this system could ever describe it. So the comparison is `contract − opening balance`. Claims invoiced INSIDE the window stay in the plan: they were planned here, and removing them would make a completed project look unplanned. Sub-dollar gaps are reported as nothing — a cent is rounding in the source, and amber on half the register for a cent teaches the eye to skip the check.
- **ADR-40 — Procurement: payment and delivery are INDEPENDENT, and a state may be asserted without a date.** Migrations `011`–`015`. Modelled from the register a project engineer fills in and accounts works from: a contract's costs arrive as line items, a quote may cover several projects and carries the FX RATE agreed with the supplier, a purchase order is generally one per project, and one supplier invoice regularly covers several orders — so the invoice links per LINE. `Paid - Pending Delivery` sits alongside `Delivered` on twelve of fifty-nine rows, which is the normal case rather than an edge one, so the events are DATES and the state is derived. But the register mostly has no dates: fourteen rows say `Delivered` and only twenty-seven carry any date at all. So `stated_state` holds a state with nothing dated behind it — whether the sheet said it or someone chose it from the grid — and `state_undated` marks those, because that is the work still to do. A real date supersedes a stated one; `when` beats `what someone said`. **The foreign amount is the fact:** `$33.00 x 7` at 1.388561 is $320.76 converting the extended total, and $320.74 converting per unit, which is why five register rows looked wrong and were not. `Db.extend` converts once, last.
- **ADR-41 — Supplier names are resolved, never guessed.** Only four of thirteen names in the procurement register match the iTrade supplier list: `USR` is Jinan USR IOT Technology Limited, `Abakus` is spelled `Abukus` in the list, `Eve` is EVE Security Services. Fuzzy matching gets `Colterlec` right and `USR` wrong, and **a wrong supplier on an order puts spend against a company that never sold us anything** — worse than a missing row, because it is wrong rather than absent. So an unmatched name aborts the import with a copy-pasteable `--alias` line, and the mapping is recorded once in `supplier_alias` so later imports match silently. Same arrangement as `job_code_alias`. A missing PROJECT is treated differently: those rows are skipped and listed, because creating a project needs a job-code decision (ADR-28) that an importer must not make.
- **ADR-42 — An importer fails before it writes.** The procurement importer validated the FX rate at the DATABASE, so a register with USD rows and no rate created twenty-two quotes and then raised — leaving a half-import that the one-shot guard then refused to replace. Validation now happens up front and the guard counts EVERY table the importer writes, not just the one it is named after. `--reset` clears a partial import deliberately, because a half-import reads as real procurement that nobody ordered.
- **ADR-43 — An ESTIMATE is not a commitment, and it is replaced in place.** Migration `016`. The Project Expenses matrix carries early estimates of future procurement, flagged in orange by whoever entered them: 31 cells, $1,576,928.29 — **ten times the value of the real orders**. Adding the two together would have made committed cost wrong by a factor of ten, so `procurement_line.is_estimate` marks them and every view separates committed from estimated from forecast. They carry no supplier, quote or order, because nothing has been quoted or approved. **An estimate is REPLACED, not deleted**: when the work is quoted the same row gains a supplier and a real cost and the flag clears, so the month keeps its forecast while somebody types the real figure. Confirmation that the flags were read correctly: from Oct-26 onward every month in the matrix is entirely estimate, matching the sheet's own monthly totals to the dollar.
- **ADR-44 — Cell colour is not data, and a legend describes intent rather than fact.** The orange flags survive no export: CSV carries no formatting and neither does the Drive conversion, so `list-orange-expenses.gs` runs inside the workbook and writes a tab that does. Two things cost time and are worth recording. The legend cell reads `#F26722`, the brand colour — but the flags are `#FF9900`, Google's palette orange, because nobody types a hex when flagging a cell, they pick the nearest swatch. The two are 97 apart in summed channel distance and a tolerance of 90 found nothing while looking at 31 orange cells. And the first script read `getBackgrounds()` when the flag is on the FONT. **A script reporting `0 found` has said nothing about why**, so it now prints every colour it did see. Same lesson as the `Phase` column meaning three different things and the register's `Purchase Order` column meaning contract value.
- **ADR-45 — `is_paid` and `is_delivered` are computed once, in the view.** The screen showed twenty rows reading `complete` or `paid - pending delivery` and reported $0.00 paid, because the figures counted `paid_date` while the state column also read the stated state. **A screen that disagrees with itself is worse than either answer alone**: whichever number someone believes, they have been given a reason to distrust the other. Both flags now come from the same source the state column uses — a date where there is one, a stated state where there is not — and both spellings count, since the register writes `paid - pending delivery` and the derived state writes `paid, pending delivery`. A cancelled line is neither however it reads, and an estimate is never paid.
- **ADR-46 — A fifth role, `finance`, for wages.** Migration `020`. Office expenses are salaries: Justin's, Richard's, everyone's superannuation. The four existing roles all imply seeing the project register, and there is no reason a project engineer entering procurement should also see what colleagues earn. `finance` implies NOTHING and is implied by nothing, **including admin** — an administrator can grant it, which is what administering is, but granting is not having. Adding a role means rebuilding `user_entity_role`, because SQLite cannot alter a CHECK. The roles are written down in three places (the constraint, `auth.ROLES`, the access module) and adding it to two of the three produced `unknown role 'finance'` from a route that had already been granted it — a 401 for a user who was, by every other account, authorised. A gate now asserts the three agree.
- **ADR-47 — A salary is the fact; the months are its consequence.** Migration `021`. The office-expense sheet stores eighteen monthly figures per person, so a pay rise has to be typed twelve times and a wage's provenance is invisible. `Finau` at $5,833.33 a month is $70,000 a year rising to $85,000 in Oct-26, and the importer recovers that. A **salary revision** carries an effective month; the monthly figure is derived; superannuation is 12% of that person's own wages; Work Cover and payroll tax are a rate on wages plus super **for the line's state**, since one employee in NSW changes both; and NSW payroll tax takes $47,000 off the year first. Change a salary and five other lines follow. **Rates are editable as percentages** and stored in hundredths of a basis point, because 1.785% and 0.39% both matter to the cent, and changing one recomputes every month — a rate that changes and does not reach the months is a rate nobody has changed. A figure typed by hand is never overwritten.
- **ADR-48 — The spreadsheet was wrong three times, and each was found by computing it.** VIC payroll tax froze at $4,255.07 when wages rose in Oct-26 while Work Cover moved with them: **$792.16 a month, $16,635.36 across the two years shown**. The NSW Work Cover line computed 0.405% while being called 0.39% — and here, unlike the `#F26722` legend (ADR-44) or the `Phase` column, **the LABEL was the correct one**, which is why it was worth asking rather than assuming the usual direction. And the sheet's own total row exceeds the sum of its own rows by a constant $526.39 before October and $1,416.56 after. A figure that has to be dragged across a row by hand is a figure that eventually is not; the platform's totals are computed at every level, so a group header is always the sum of its lines.
- **ADR-49 — Seeing what the business costs is not seeing what people earn.** Migration `022`. `finance` opens the office-expense screen: rent, subscriptions, payroll tax, the total. Reporting will need it and more than one person will have it. `payroll` is a SIXTH role for individual salaries, implied by nothing — not by `finance`, not by `admin`. Somebody can see that wages cost $96,250.01 in July without seeing that Justin is on $215,000. An administrator grants it, and that an administrator could grant it to themselves is true and is the point of the audit log: **the control is that it is recorded, not that it is impossible.**
- **ADR-50 — A salary is WITHHELD, not hidden.** Hiding a figure in the interface hides it from nobody who matters: a salary the browser is sent is a salary anyone with the developer tools already has. So `annual_cents` is removed from the response, along with the revision history, which is the salary month by month. The screen shows dots and asks. **There is no password in this platform** — sign-in is Google — so demanding one means demanding a FRESH Google authentication: `/auth/elevate` sends `prompt=login`, which makes Google ask again despite a live session. The elevation is a separate short-lived cookie bound to the user, not a flag on the session, so its absence is the normal state and it lapses on its own after fifteen minutes. Every salary viewed is audited, because a control that leaves no trace is one nobody can check was working. Statutory RATES stay visible: a rate is policy rather than pay, and hiding it leaves the charge beneath it unauditable.
- **ADR-51 — Corporate tax is assessed on the YEAR, and office cost never touches a project.** Migration `023`. Two rules the dashboard turns on, and the workbook got both wrong. **Tax:** it taxed each profitable month at 25% with no credit for loss months, giving $267,227 against $104,647 of gross profit and a headline net profit of MINUS $162,580 — while the block beside it taxed the year once for $26,162 and PLUS $78,485. A quarter of a million apart. Company tax is assessed on the year: a loss in May offsets a profit in December, and a year that loses money pays nothing. No carry-forward between years, because a forecast that assumes relief it has not claimed flatters itself. **Overhead:** rent and payroll tax are not bought for a job, so spreading them across jobs would invent a margin nobody agreed to. They come off the bottom line. The tax RATE and the further-sales overlay live in `fy_settings` per financial year, because both change per financial year and a rate baked into a formula is one somebody has to find under pressure. Changing them is `admin`, not `finance`: they change what every figure on the screen means.
- **ADR-52 — A month that has ended is a fact; the rest is a projection, and the screen says which.** The dashboard marks actual months and draws projected bars faded. Revenue is split into billed and forecast, project cost into committed and estimated, and the project-by-month grid colours a billed month in the accent. A dashboard that mixes what happened with what is expected, without saying so, is one nobody can act on — and the workbook mixed them silently in every row.
- **ADR-53 — THE SHEET MAY NOT UNDO WHAT THE PLATFORM RECORDED.** The general form of a rule that had already been written three times separately, and the resync showed why it has to be one rule. A procurement sync would have walked nineteen lines backwards from `complete` to `delivered` — not because the register had changed, but because the Ops Manager had marked them complete HERE and the register still said what it said at the last export. So: a sync may tell the platform something it has never been told, and may not overwrite something recorded here. The test is the revision history, not a guess. Already true of `delivered_date` and `paid_date`, which the register does not hold at all; of an ENTERED expense figure beating a calculated one; and of an invoiced claim, which a re-spread plan cannot move. **The platform is the record and the sheet is an input to it**, which is the whole point of the migration and had to be made explicit before the first resync could be trusted.
- **ADR-54 — A rate is fixed at quote, and the sheet's rate cell is not.** The procurement sheet holds a LIVE exchange rate: opening the file re-converted every USD line from 1.388561 to 1.400606, moving twelve costs without a single price changing. The rate is agreed with the supplier and fixed at quote (ADR-40), so the sync costs each existing line at ITS OWN quote's rate rather than at whatever the sheet says today — and for a line imported without a quote reference, at the rate recovered exactly from what it was costed at. The result is the same whichever `--fx-rate` is passed, which is the property worth having: **a figure that changes because somebody opened a spreadsheet is not a figure.**
- **ADR-55 — A vocabulary belongs in a table, not a CHECK.** Migration `024`, and the first `.nofk.sql` migration. The register began using `Lost` for work tendered and not won; `status` was a CHECK over four values, and widening one means rebuilding a table that eight others reference — which, with foreign keys on, simply fails. So the runner gained a rebuild mode: keys off, every view dropped and restored around it, and `foreign_key_check` BEFORE the commit, so a rebuild that orphans a row is refused rather than discovered later. Done once and properly: statuses are now a lookup with `is_open`, so adding one is an INSERT and the dashboard counts on the flag rather than on the literal `Active`. Writing the rebuild from memory dropped four columns, invented a UNIQUE index that would have refused data the platform already holds, and lost the ADR-22 CHECK — so a gate now compares a rebuilt table against what it replaced.
- **ADR-56 — DELETE is for a row that should never have existed; CANCEL is for one that was real.** Found by the Ops Manager on `The Lindrum - ICN`: a $44,000 expense estimate superseded by the $39,600 purchase that replaced it, with nothing to remove the estimate. Cancelling would have been wrong — it keeps the line visible as something that was going to happen and stopped, which is not what a duplicate is. So both exist and the dialog says which is which, because they are one click apart. **Money that moved cannot be deleted:** a line invoiced, paid, delivered or attached to a supplier invoice is refused with the reason, and cancelled instead. A reason is mandatory and the WHOLE ROW is written to the audit log before it goes — project, supplier, item, quantity, amount, month, state — because a deletion nobody can reconstruct is a deletion nobody can question. Approver rather than operations: deleting is the one action that leaves nothing on the screen to notice.
- **ADR-57 — Forecast coverage is a per-project fact, and it is named as one.** `orders in hand` is contract less invoiced: everything still to bill. The forecast is what has been put into a month. If the forecasting is complete they agree, and where they do not the difference is work nobody has scheduled or a month carrying more than the contract allows. Counted PER PROJECT and never netted: a portfolio total would cancel an under-forecast job against an over-forecast one and report that all was well. On the live data, 59 of 65 projects are covered, $2,800 sits in no month, and $102,180 is over — almost all of it the `88 Robertson St` variation already on the worklist. **The wording matters as much as the figure:** the column reads on a claim row, so `not forecast` beside a forecast claim was a contradiction, and it now reads `under $2,000.00` against the PROJECT. A dollar is rounding rather than a finding; amber on half the register teaches the eye to skip the check.
- **ADR-58 — Charts take their colours from CSS, because the CSP blocks inline styles.** `default-src 'self'` drops a `style` attribute set from a script SILENTLY: the chart legend rendered grey beside correctly coloured bars, because bars use the SVG `fill` attribute and the legend used CSS. Nothing errored. Series colours are now one CSS rule each, covering the bar, the line, the point and the swatch together so a colour cannot drift between them, and a gate refuses any computed `style:` in a screen — and asserts the policy is still what that assumes. The charts themselves are four hundred lines of SVG rather than a library: ninety kilobytes to draw rectangles, bringing its own type scale and palette to argue with ours, is a trade this platform does not make (§7).
- **ADR-59 — A backup that never runs is worse than no backup, because it is believed.** The hourly `VACUUM INTO` snapshotter had been running since the first boot and every snapshot sat on the disk it protected. `offbox_sync.ps1` is the Windows counterpart to the VM's `offbox_sync.sh`: it refuses a destination on the same volume as the source, copies `backups/` and `documents/` and NEVER the live `ops.db` — a WAL database copied mid-transaction yields a `.db` and a `-wal` that disagree, and the copy fails only at restore, on the day you need it. **The first version would have failed silently every hour:** `-Source` defaulted to the relative path `data`, and a scheduled task runs from `system32`, so it resolved to `C:\Windows\system32\data` while the task showed as Ready. The path now comes from `$PSScriptRoot` and is printed on every run. Windows scheduled tasks also default to stopping on battery, which for a laptop means not running. Gates cover both scripts for the live-database copy and the working-directory assumption; the restore is proven with `--check`, which asserts the register still reconciles rather than only that a file was copied.
